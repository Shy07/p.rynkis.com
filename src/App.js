...
class App extends Component {
  ...
  componentDidMount() {
    window.$.ajax({
      ...
      url: "http://192.168.0.103:4000/todays/poetry",
      dataType: 'json',
      type: 'GET',
      cache: false,
      success: (data) => {
  ...
  render() {
    ...
    if (poetry !== null) {
      ...
      let commits = null;
      if (poetry.commit.length > 0) {
        let commit = poetry.commit.map((c, i) => (<small key={i}>{c}<br/></small>));
        commits = (
          <div>
        ...
        <div>
          <div ref='content' className="content">
            ...
            <div className='title'>{poetry.title}
              <div className='author'>{poetry.author}<span>詩</span></div>
            </div>
          </div>
          {commits}
        </div>
